...
# NumPy 2.x compatibility shim for older XGBoost (expects np.cfloat, etc.)
...
# reduce noisy datetime parsing warnings from feature_engine / pandas
...
def part_of_day(X, morning=4, noon=12, eve=16, night=20):
	columns = X.columns.to_list()
	X_temp = X.assign(**{
		# dep/arrival time is always an HH:MM:SS string -> parse with explicit format
		col: pd.to_datetime(X.loc[:, col], format="%H:%M:%S", errors="coerce").dt.hour
		for col in columns
	})
...
if st.button("Predict"):
	saved_preprocessor = get_or_create_preprocessor()
	x_new_pre = saved_preprocessor.transform(x_new)

	with open("xgboost-model", "rb") as f:
		model = pickle.load(f)
	# Avoid XGBoost↔pandas and NumPy 2.x copy issues
	x_new_xgb = xgb.DMatrix(x_new_pre.to_numpy(dtype=np.float32))
